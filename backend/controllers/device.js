const {
  ...
  exports.createDevice = async (req, res) => {
    try {
      ...
      const data = req.body; 
      const deviceData = await createDeviceServices({...data,userId:req.user.id});
      return res.status(200).json({ message: "Device created", data: deviceData });
    } catch (err) {
      return res
        .status(500)
        .json({ message: "Internal server error", error: err.message });
  ...
  exports.updateDevice = async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: "Device updated", data: deviceData });
    } catch (err) {
      return res
        .status(500)
        .json({ message: "Internal server error", error: err.message });
  ...
  exports.deleteDevice = async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: "Device deleted", data: deviceData });
    } catch (err) {
      return res
        .status(500)
        .json({ message: "Internal server error", error: err.message });
