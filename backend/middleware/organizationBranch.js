...
exports.validateCreateOrganizationBranchData = [
    body('organization').isMongoId().withMessage("Organization ID must be a valid MongoDB ObjectId"),
    body('branchName').isString().notEmpty().withMessage("Branch name must be a non-empty string"),
    body('primaryAddress').isString().notEmpty().withMessage("Primary address must be a non-empty string"),
    ...
    body('district').isString().notEmpty().withMessage("District must be a non-empty string"),
    body('state').isString().notEmpty().withMessage("State must be a non-empty string"),
    body('zipCode').isString().notEmpty().withMessage("Zip code must be a non-empty string"),
    ...
    body('mobile').isString().notEmpty().withMessage("Mobile number must be a non-empty string"),
    body('email').isEmail().withMessage("Email must be a valid email address"),
    body('companyType').isString().notEmpty().withMessage("Company type must be a non-empty string"),
    (req, res, next) => {
...
exports.validateUpdateOrganizationBranchData = [
    ...
    body('primaryAddress').optional().isString().withMessage("Primary address must be a string"),
    ...
    body('mobile').optional().isString().withMessage("Mobile number must be a string"),
    body('email').optional().isEmail().withMessage("Email must be a valid email address"),
    body('companyType').optional().isString().withMessage("Company type must be a string"),
    (req, res, next) => {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
